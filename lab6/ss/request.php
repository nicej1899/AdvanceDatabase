...
*/
$bd = 'localhost/CNDB';
$connection = OCI_connect('ora00097', '5vqGzP', $bd);
if(!$connection) 
        {
...
if( isset($_REQUEST['pa_id1']) && !empty($_REQUEST['pa_id1']))
{
	$pa_id1 = $_REQUEST['pa_id1'];
}

if( isset($_REQUEST['part_id']) && !empty($_REQUEST['part_id']))
{
	$part_id = $_REQUEST['part_id'];
}


$curs1 = OCI_parse($connection, "UPDATE responsible SET emp_num = $pa_id1 WHERE part_id = $part_id and emp_num = $pa_id");
$curs2 = OCI_parse($connection, "SELECT emp_num, part_id FROM responsible WHERE part_id = $part_id and emp_num = $pa_id1");
...
*/
OCI_Define_By_Name($curs2,"EMP_NUM",$emp_num);
OCI_Define_By_Name($curs2,"PART_ID",$part_id);

/*      4. Execution of the SQL request with an immediate commit to free locks */
OCI_Execute($curs1, OCI_COMMIT_ON_SUCCESS);
OCI_Execute($curs2, OCI_COMMIT_ON_SUCCESS);
$chain .= "<b>EMP_NUM  PART_ID</b><br>\n";

/*      5. Read each row from the result of the Sql request */  
while (OCI_fetch($curs2))
		$chain .= "$emp_num  &nbsp &nbsp &nbsp &nbsp &nbsp $part_id<br>\n";
